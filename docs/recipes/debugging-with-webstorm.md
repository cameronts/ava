...
Add a new *Node.js Run/Debug configuration*: select `Edit Configurations...` (or `Add Configuration` if `Edit Configurations` is not an option) from the dropdown list on the top right, then click `+` and select *Node.js*.

In the `JavaScript file` field specify the path to AVA in the project's (or user's) `node_modules` folder: `node_modules/.bin/ava` on macOS and Linux or `node_modules/.bin/ava.cmd` on Windows or `node_modules/.bin/ava` on Linux.
