...
In the `JavaScript file` field specify the path to AVA in the project's (or user's) `node_modules` folder: `node_modules/.bin/ava` on macOS and Linux or `node_modules/.bin/ava.cmd` on Windows.
